...
const HR = () => {
    ...
    const fetchApplicants = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admin/applicants/all"
        );
  ...
  useEffect(() => {
    ...
    const fetchTeams = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admin/team/all"
        );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/admin/team/add",
        formData
      );
  ...
  const TeamDetailsModal = ({ teamMember }) => (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-opacity-75 bg-gray-500">
      <div className="bg-white w-1/2 p-8 rounded-lg shadow-lg ">
        ...
        <p>
          <strong>Date of Birth:</strong> {teamMember.dob}
        </p>
        <p>
          <strong>Gender:</strong> {teamMember.gender}
        </p>
        <p>
          <strong>Blood Group:</strong> {teamMember.bloodGroup}
        </p>
        <p>
          <strong>Position:</strong> {teamMember.position}
        </p>
        <p>
          <strong>Joining Year:</strong> {teamMember.joiningYear}
        </p>
        <p>
          <strong>Mobile No.:</strong> {teamMember.mobileNo}
        </p>
        <p>
          <strong>Address:</strong> {teamMember.address}
        </p>
        <div className="mt-4">
  ...
  const saveEmployeeStatus = async () => {
    ...
    try {
      const id = selectedTeamMember.userId;
      const status = action;
      ...
      const response = await axios.put(
        `http://localhost:5000/admin/updateTeamStatus/${id}`,
        { status }
      );
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-opacity-50 bg-gray-900 ">
      <div className="bg-white w-full md:w-3/4 lg:w-1/2 p-6 rounded-lg shadow-lg max-h-full overflow-y-auto">
        ...
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-700 mb-2">
            <strong>Open Positions:</strong> {job.jobOpenPositions}
          </p>
        </div>
        <div className="mb-4">
  ...
  const handleOpeningsSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/admin/job/add",
        formData
      );
  ...
  return (
    <>
      ...
      {selectedTeamMember && (
        <TeamDetailsModal teamMember={selectedTeamMember} />
      )}
      {selectedApplicant && (
        <ApplicantDetailsModal applicant={selectedApplicant} />
      )}
