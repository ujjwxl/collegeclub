...
export const fetchEventsByTarget = async (req, res) => {
  const { target } = req.params; // Assuming 'target' is passed as a route parameter

  try {
    const eventsCollectionRef = collection(db, 'events');
    const q = query(eventsCollectionRef, where('targets', 'array-contains', target));
    const querySnapshot = await getDocs(q);

    if (querySnapshot.empty) {
      console.log(`No events found for target: ${target}`);
      return res.status(404).json({ message: `No events found for target: ${target}` });
    }
    ...
    console.error(`Error fetching events for target ${target}:`, error.message);
